...
function constructPlotData(dataType,dataSet,className){
    ...
    if($(".input-row-group-multiple-select option:selected").val() !="" && $(".input-column-group-multiple-select option:selected").val() !=""){
        ...
        //alert($('.rowuniqueNum' +selectedRowVal+' select option[value='+selectedRowVal+']').text());
    }
...
function drawChart(dataMap){
   ...
   // alert(JSON.stringify(heirarchy));
...
/*function createScales(dataset, chartConfig, dataTable){
    ...
    yScale = d3.scale.linear()
        .range([height, chartConfig.top])
        .domain([0, dataset.value]).nice();
   ...
   *//* if(dataTable.metadata.types[chartConfig.yAxisData] == 'N'){
    ...
    return { "xScale":xScale, "yScale":yScale, "colorScale":color }
